### This network was revised from MONAI code, the fusion-module and KRFormer needs to be ref from other files.
### main idea: swintransformer class forward defines the data flow, from patch+embed to all layers.
### layers was driven by modulelist which is swintransformer block
### I rewrite the encoder1 and encoder 2 to complex desined model. last encoder to be our former
### only harsh thing is that 如何嵌入自己的module进去，看看honghui的代码878行
...
from .KRFormer import KRtransformer
...
class SwinUNETR(nn.Module):
    ...
    def __init__(
        ...
        self.KRtransformer = KRtransformer(
            in_channels,
            out_channels, 
            image_size=(64,64,64), 
            layer_depth=2, 
    ...
    def forward(self, x_in):
        ...
        enc1 = self.encoder2(x_in)
        enc2 = self.encoder3(x_in)
        enc3 = self.encoder4(hidden_states_out[0])
        enc4 = self.encoder5(hidden_states_out[1])
...
class SwinTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.former = KRtransformer(32, 32, layer_depth=2, patch_size=2, heads=6, dim_head=128, dropout=0.1, emb_dropout=0.1)
